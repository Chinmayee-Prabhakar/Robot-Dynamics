...
Ib1 = [(m1*(w*w + L1*L1)/12) 0 0;0 (m1*(l*l + L1*L1)/12) 0;0 0 (m1*(w*w + l*l)/12)];
Ib2 = [(m2*(w*w + L2*L2)/12) 0 0;0 (m2*(L2*L2 + l*l)/12) 0;0 0 (m2*(w*w + l*l)/12)];
Ib3 = [m3*(w*w + L3*L3)/12 0 0;0 m3*(L3*L3 + l*l)/12 0;0 0 m3*(w*w + l*l)/12];

G1 = [Ib1 zeros(3,3); zeros(3,3) (m1*eye(3))];  %Spatial Inertia Matrix for link 1
G2 = [Ib2 zeros(3,3); zeros(3,3) (m1*eye(3))];  %Spatial Inertia Matrix for link 2
G3 = [Ib3 zeros(3,3); zeros(3,3) (m1*eye(3))];  %Spatial Inertia Matrix for link 3
...
for jj = 1 : nPts - 1
    ...
    Ftip = [0 0 0 0 -(1*9.81) 0];
    Ftipmat = repmat(Ftip,N,1);
